...
import ModalSintetico from "@/components/ModalSintetico";
...
export default function PainelGestao() {
  ...
  const [sinteticoAberto, setSinteticoAberto] = useState(false);
  ...
  // Os cards e a tabela vêm da mesma consulta: alterar qualquer filtro
  // recalcula os dois juntos e volta para a primeira página.
  useEffect(() => {
    let cancelado = false;
    setCarregando(true);
    setErro(null);
  ...
  // ---- paginação (cards e relatórios seguem sobre o filtro inteiro) -------
  ...
  const criterios = [
    ...
    `Tipo: ${tipo === "OPERACAO" ? "Operação" : tipo === "PAUSA" ? "Pausa" : "Todos"}   |   Data: ${
      data ? formatarData(`${data}T12:00:00`) : "Todas"
    }`,
  ...
  return (
    <main className="painel">
      <div className="painel-topo">
        <h1 className="painel-titulo">Painel de Gestão</h1>
        <Link
          href="/"
          className="botao botao--neutro"
          style={{ lineHeight: "34px", textDecoration: "none" }}
        >
          Tela de apontamento
        </Link>
      </div>
      ...
      <section className="cartao">
        <div className="cartao-cabecalho">
          <h2 className="cartao-titulo">Apontamentos</h2>
          <div className="linha-acoes">
            <button
              type="button"
              className="botao"
              disabled={linhas.length === 0}
              onClick={() => gerarPdfAnalitico(linhas, totais, criterios)}
            >
              Relatório Analítico
            </button>
            <button type="button" className="botao" onClick={() => setSinteticoAberto(true)}>
              Relatório Sintético
            </button>
          </div>
        </div>
      ...
      <GestaoEtapas etapas={etapas} aoMudar={carregarEtapas} />

      {sinteticoAberto && (
        <ModalSintetico
          etapas={etapas}
          filtrosAtuais={{ os, etapaId, data }}
          aoFechar={() => setSinteticoAberto(false)}
        />
      )}
    </main>
  );
