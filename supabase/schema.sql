...
create table if not exists apontamentos (
  id uuid primary key default gen_random_uuid(),
  -- Contador sequencial por ordem de criacao: e o "#" da tabela do painel.
...
begin
  if not exists (
    select 1 from information_schema.columns
     where table_name = 'apontamentos' and column_name = 'numero'
  ) then
    alter table apontamentos add column numero bigint generated by default as identity;
    with ordenados as (
      select id, row_number() over (order by inicio, id) as seq from apontamentos
    )
    update apontamentos a set numero = o.seq from ordenados o where o.id = a.id;
  end if;
end $$;
